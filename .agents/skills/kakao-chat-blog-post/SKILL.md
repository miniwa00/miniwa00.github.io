...
초안은 대화 내용을 단순 요약하지 않는다. 승인된 주제의식이 글 전체를 관통해야 하며, 각 문단은 그 주제의식을 다른 각도에서 밀어주는 역할을 해야 한다.

글을 적절하게 쪼개서 Markdown 헤더(`##`, `###`)로 구분한다. 너무 긴 단락은 적절히 나누고, 각 단락이 하나의 핵심 아이디어를 담도록 한다.

각 섹션별로 적절한 소제목을 달아준다. 소제목은 해당 섹션의 핵심 아이디어를 짧게 요약하는 형태로 작성한다.
...
10. 커밋 메시지는 반드시 prefix를 붙인다. 포스트 게시에는 기본적으로 `docs: <포스트 제목>`을 사용하고, 스킬/도구 수정에는 `chore: <변경 요약>` 또는 `fix: <변경 요약>`을 사용한다. 현재 repo/스레드의 git 지시가 있으면 prefix는 유지한 채 언어, 어미, 형식 같은 세부 규칙을 함께 반영한다.
...
```bash
git add <필요한 파일>
git commit -m "docs: <현재 git 지시에 맞는 메시지>"
